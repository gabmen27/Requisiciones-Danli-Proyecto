...
export default function AprobacionAlcaldePage() {
  ...
  useEffect(() => {
    ...
    if (!cargando && user && !["alcaldia", "admin"].includes(user.rol)) {
  ...
  const handleAprobar = async (req: Requisicion) => {
    ...
    setBusy(req.id);
    try {
      await aprobarRequisicion(req.id, "alcaldia");
    ...
    <div className="flex h-screen overflow-hidden bg-gray-100">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        <header className="bg-white border-b border-gray-200 px-8 py-4 sticky top-0 z-10">
          <p className="text-xs font-semibold tracking-widest text-gray-400 uppercase">Alcalde</p>
