...
export default function RequisicionesPage() {
  ...
  const handleEnviarAprobacion = async (id: number) => {
    try {
      ...
      setError(err.message || 'Error al enviar a aprobacion');
    }
  };
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-64"><div className="text-gray-500">Cargando...</div></div>;
  }
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/dashboard/requisiciones/nueva">
          <Button variant="primary">+ Nueva Requisicion</Button>
        </Link>
      </div>
      {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">{error}</div>}
      <Table headers={['Numero', 'Tipo', 'Estado', 'Total', 'Fecha', 'Acciones']}>
        {requisiciones.length === 0 ? (
          <tr><td colSpan={6} className="px-6 py-4 text-center text-gray-500">No hay requisiciones</td></tr>
        ) : (
          requisiciones.map((req) => (
            <tr key={req.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm">
                <Link href={"/dashboard/requisiciones/" + req.id} className="text-primary hover:underline mr-3">Ver</Link>
                {req.estado === 'borrador' && (
                  <button onClick={() => handleEnviarAprobacion(req.id)} className="text-secondary hover:underline mr-3">Enviar</button>
                )}
                {req.estado === 'pendiente' && (user?.rol === 'gerencia' || user?.rol === 'alcaldia' || user?.rol === 'admin') && (
                  <>
                    <button onClick={() => handleAprobar(req.id)} className="text-success hover:underline mr-3">Aprobar</button>
                    <button onClick={() => handleRechazar(req.id)} className="text-danger hover:underline">Rechazar</button>
                  </>
                )}
              </td>
            </tr>
